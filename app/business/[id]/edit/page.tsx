...
// Uploaded media may be a path relative to the API origin — same helper as
// the business detail page's getMediaUrl.
const getMediaUrl = (path: string) => {
  if (path.startsWith("http://") || path.startsWith("https://")) return path;
  return `${process.env.NEXT_PUBLIC_API_URL}${path}`;
};

// Same form as create (app/business/create/page.tsx), prefilled and
// PATCHing instead of POSTing. See the room edit page for the note on why
// existing vs. new media is tracked separately.
export default function EditBusinessPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (profile.id !== business.ownerId) {
          setIsOwner(false);
          return;
        }
        ...
        setSupportsDelivery(business.supportsDelivery);
        setSupportsWhatsAppOrder(business.supportsWhatsAppOrder);
        setExistingImages(business.images ?? []);
        setExistingVideos(business.videos ?? []);
        setForm({
          name: business.name,
  ...
  if (!isLoggedIn) {
    return (
      <main className="relative min-h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 px-4 pt-24">
        <AuthGate
          nextPath={`/business/${id}/edit`}
          title="Sign in to edit your business"
  ...
  if (notFound || !isOwner) {
    return (
      <main className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 px-4 pt-24">
        <div className="flex items-center justify-center py-32 text-gray-500">
          {notFound ? "Business not found." : "You can only edit your own listings."}
        </div>
      </main>
    );
  }
